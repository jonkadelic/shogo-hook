...
bool world__init(
    ...
    for (size_t i = 0; i < self->models_len; i++) {
        ...
        world_model->texture = shared_texture_manager__get_texture(textures, shared_texture_cache[i]);
        if (world_model->texture == nullptr) {
            LOG_ERROR("Failed to get texture for world model mesh");
            goto err;
        }

        if (!build_world_model(self, main_world->m_pWorldBsp, i, tessellator)) {
